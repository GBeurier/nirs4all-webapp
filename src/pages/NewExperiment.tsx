...
export default function NewExperiment() {
  ...
  // Auto-generate experiment name: "dataset x pipeline" or "Corn_Whea x PLS_SVR" for multiples
  const autoGeneratedName = useMemo(() => {
    ...
    const summarize = (names: string[]) =>
      names.length === 1
        ? names[0]
        : names.map(n => n.slice(0, 4)).join("_");
  ...
  // Pre-fill experiment name when entering step 3
  useEffect(() => {
    if (currentStep === 3 && customExperimentName === "" && autoGeneratedName) {
